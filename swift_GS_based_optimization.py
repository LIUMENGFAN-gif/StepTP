...
class GSNode:
    ...
    def best_child(self):
        return max(self.children, key=lambda child: child.speedup)
...
class GS:
  ...
  def search(self, original_IR, iterations):
    ...
    root = GSNode(state=original_IR, state_dict={"original_IR": original_IR})
    current_node = root
  ...
  def _expand(self, node):
    ...
    for answer_dict in answer_list:
      ...
      target_node_list.append(GSNode(state=transformed_IR, state_dict=answer_dict, depth=node.depth+1))
...
if __name__ == '__main__':
    ...
    #--------------pre-settings----------------------------
    step_by_step_prompt="Greedy-Search-based optimization is used on a given IR to improve performance. Each IR is a state, and has a parent transformation and speedup performance."
    ...
    error_record_dir=f'../nfs_folder/evaluation/step_by_step/GS_based/{args.GPU}/error_record_{model_size}.json'
    ...
    for idx in tqdm.tqdm(range(args.start_data_idx, args.end_data_idx)):
        ...
        # print("done")
        gs=GS(info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, 0, idx, database, error_record_dir)
        print("GS created")
        optimized_IR, speedup, final_node, database=gs.search(original_IR, max_iteration)
        ...
        with open(f'../nfs_folder/evaluation/step_by_step/GS_based/{args.GPU}/result/result_{idx}.json', 'w') as f:
            json.dump({'optimized_IR':optimized_IR, 'speedup':speedup}, f)
        with open(f'../nfs_folder/evaluation/step_by_step/GS_based/{args.GPU}/database/database_{idx}.json', 'w') as f:
            json.dump(database, f)
        with open(f'../nfs_folder/evaluation/step_by_step/GS_based/{args.GPU}/node_history/history_{idx}.json', 'w') as f:
