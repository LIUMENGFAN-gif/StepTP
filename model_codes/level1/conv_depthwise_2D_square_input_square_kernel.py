...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, kernel_size: int, stride: int = 1, padding: int = 0, bias: bool = False, dtype: torch.dtype = torch.float16):
...
def get_model(in_channels=3, kernel_size=3, stride=1, padding=0, bias=False, dtype=torch.float16):
    return Model(in_channels, kernel_size, stride, padding, bias, dtype)
...
def set_default_shapes_ranges_and_dtypes():
    ...
    return [in_channels, kernel_size, stride, padding, batch_size, height, width], \
           [range_in_channels, range_kernel_size, range_stride, range_padding, range_batch_size, range_height, range_width], \
           dtype

def split_shapes_into_input_and_model_params_shapes(in_channels, kernel_size, stride, padding, batch_size, height, width):
    return [batch_size, in_channels, height, width], \
