...
class Model(nn.Module):
    """
    Simple model that performs a single matrix multiplication (C = A * B) with both A and B transposed.
    """
    def __init__(self):
...
def get_default_input_shapes():
    ...
    The shapes are (K, M) for A and (N, K) for B.
    """
    K = 512
    M = 1024
    N = 768
...
def get_inputs(K, M, N, dtype=torch.float16):
    ...
    Returns:
        list: [A, B] where A is (K, M) and B is (N, K), both on device='meta'.
    """
