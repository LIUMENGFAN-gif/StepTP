...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_features, out_features, bias_shape, dtype=torch.float16):
        super(Model, self).__init__()
        self.gemm = nn.Linear(in_features, out_features, bias=False).to(dtype)
...
def get_model(in_features=1024, out_features=512, bias_shape=[512], dtype=torch.float16):
    return Model(in_features, out_features, bias_shape, dtype=dtype)
