...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, sum_tensor_shape, dtype=torch.float16):
        super(Model, self).__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, bias=False).to(dtype)
        self.sum_tensor = nn.Parameter(torch.randn(*sum_tensor_shape, dtype=dtype))
...
def get_model(in_channels=3, out_channels=16, kernel_size=3, sum_tensor_shape=[16, 1, 1, 1], dtype=torch.float16):
    return Model(in_channels, out_channels, kernel_size, sum_tensor_shape, dtype=dtype)
