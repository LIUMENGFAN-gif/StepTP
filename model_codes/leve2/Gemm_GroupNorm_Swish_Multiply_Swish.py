...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_features, out_features, num_groups, multiply_weight_shape, dtype=torch.float16):
        super(Model, self).__init__()
        self.gemm = nn.Linear(in_features, out_features).to(dtype)
        self.group_norm = nn.GroupNorm(num_groups, out_features).to(dtype)
        self.multiply_weight = nn.Parameter(torch.randn(*multiply_weight_shape, dtype=dtype))
...
def get_model(in_features=512, out_features=1024, num_groups=16, multiply_weight_shape=[1024], dtype=torch.float16):
    return Model(in_features, out_features, num_groups, multiply_weight_shape, dtype=dtype)
...
def split_shapes_into_input_and_model_params_shapes(in_features, out_features, num_groups, batch_size):
    return [batch_size, in_features], \
           [in_features, out_features, num_groups, [out_features]]