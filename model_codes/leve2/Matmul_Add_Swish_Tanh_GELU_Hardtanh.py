...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_features, out_features, add_value_shape, dtype=torch.float16):
        super(Model, self).__init__()
        self.matmul = nn.Linear(in_features, out_features).to(dtype)
        self.add_value = nn.Parameter(torch.randn(*add_value_shape, dtype=dtype))
...
def get_model(in_features=1024, out_features=512, add_value_shape=[512], dtype=torch.float16):
    return Model(in_features, out_features, add_value_shape, dtype=dtype)
