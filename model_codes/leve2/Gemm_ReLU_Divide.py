...
class Model(nn.Module):
    ...
    """
    def __init__(self, in_features, out_features, divisor, dtype=torch.float16):
...
def get_model(in_features, out_features, divisor, dtype=torch.float16):
    return Model(in_features, out_features, divisor, dtype=dtype)
...
def set_default_shapes_ranges_and_dtypes():
    ...
    range_divisor = [1.0, 10.0]
    range_batch_size = [1, 1024]
    dtype = [int, int, float, int]
    return [in_features, out_features, divisor, batch_size], \
           [range_in_features, range_out_features, range_divisor, range_batch_size], dtype

def split_shapes_into_input_and_model_params_shapes(in_features, out_features, divisor, batch_size):
    return [batch_size, in_features], \
