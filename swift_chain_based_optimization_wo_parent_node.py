...
def construct_prompt(node, step_by_step_prompt, start_description, basic_description, requirement, strategy_prompt, hardware_info):
    ...
    if node.trials>0:
        ...
        current_info+='.\n'
    return step_by_step_prompt+"\n"+start_description+": \'"+current_IR+"\', "+basic_description+current_info+hardware_info+strategy_prompt+requirement

class chain_based_Node:
...
class chain_based:
  ...
  def search(self, original_IR, iterations):
    ...
    current_node = chain_based_Node(state=original_IR, state_dict={"original_IR": original_IR})
    for _ in range(iterations):
      #---------------------expand--------------------
      if not self._is_terminal():
        print(f'current_node:{current_node}')
        target_node, IR_in_prompt = self._expand(current_node)
        #---------------------simulation----------------
        verification_result = self._simulate(target_node, current_node.state)
        #---------------------check_children-------------
        self._check_children(current_node, target_node, verification_result)
        #---------------------selection-----------------
  ...
  def _check_children(self, current_node, target_node, verification_result):
  ...
  def _expand(self, node):
    ...
    target_node=chain_based_Node(state=transformed_IR, state_dict=answer_dict, depth=node.depth+1)
...
if __name__ == '__main__':
    ...
    #--------------pre-settings----------------------------
    step_by_step_prompt="Chain-based optimization is used on a given IR to improve performance. Each IR is a state, and has a parent transformation and speedup performance."
    ...
    error_record_dir=f'../nfs_folder/evaluation/step_by_step/chain_based_wo_parent/{args.GPU}/error_record_{model_size}.json'
    ...
    for idx in tqdm.tqdm(range(args.start_data_idx, args.end_data_idx)):
        ...
        cb=chain_based(info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, 0, idx, database, error_record_dir)
        print("chain_based infer created")
        optimized_IR, speedup, final_node, database=cb.search(original_IR, max_iterations)
        ...
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based_wo_parent/{args.GPU}/result/result_{idx}.json', 'w') as f:
            json.dump({'optimized_IR':optimized_IR, 'speedup':speedup}, f)
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based_wo_parent/{args.GPU}/database/database_{idx}.json', 'w') as f:
            json.dump(database, f)
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based_wo_parent/{args.GPU}/node_history/history_{idx}.json', 'w') as f:
