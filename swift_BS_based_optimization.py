...
class BS:
  def __init__(self, k, info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, veri_cuda, eval_idx, database, error_record_dir):
    self.k = k
    self.info = info
  ...
  def search(self, original_IR, iterations):
    ...
    root=BSNode(state=original_IR, state_dict={"original_IR": original_IR})
    ...
    for _ in range(iterations):
      ...
      if not self._is_terminal():
        ...
        #---------------------selection-----------------
        current_node_list = self._select(current_node_list)
    try:
  ...
  def _expand(self, node):
    ...
    for answer_dict in answer_list:
      ...
      target_node_list.append(BSNode(state=transformed_IR, state_dict=answer_dict, depth=node.depth+1))
  ...
  def best_topk_child(self, children_list):
      return heapq.nlargest(self.k, children_list, key=lambda child: child.speedup)

  def _select(self, current_node_list):
    ...
    node_list = self.best_topk_child(children_list)
    return node_list
  ...
  def _is_terminal(self):
    ...
    """
    if self.repeat_num<=5*self.num_IRs:
        return False
    else:
        return True
...
if __name__ == '__main__':
    ...
    #--------------pre-settings----------------------------
    step_by_step_prompt="Beam-Search-based optimization is used on a given IR to improve performance. Each IR is a state, and has a parent transformation and speedup performance."
    ...
    error_record_dir=f'../nfs_folder/evaluation/step_by_step/BS_based/{args.GPU}/error_record_{model_size}.json'
    ...
    for idx in tqdm.tqdm(range(args.start_data_idx, args.end_data_idx)):
        ...
        # print("done")
        bs=BS(topk, info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, 0, idx, database, error_record_dir)
        print("BS created")
        optimized_IR, speedup, final_node, database=bs.search(original_IR, max_iterations)
        ...
        with open(f'../nfs_folder/evaluation/step_by_step/BS_based/{args.GPU}/result/result_{idx}.json', 'w') as f:
            json.dump({'optimized_IR':optimized_IR, 'speedup':speedup}, f)
        with open(f'../nfs_folder/evaluation/step_by_step/BS_based/{args.GPU}/database/database_{idx}.json', 'w') as f:
            json.dump(database, f)
        with open(f'../nfs_folder/evaluation/step_by_step/BS_based/{args.GPU}/node_history/history_{idx}.json', 'w') as f:
