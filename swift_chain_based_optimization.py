...
class chain_based:
  ...
  def search(self, original_IR, iterations):
    ...
    root = chain_based_Node(state=original_IR, state_dict={"original_IR": original_IR})
    current_node = root
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start_data_idx", type=int, default=48)
    parser.add_argument("--end_data_idx", type=int, default=4)
    ...
    error_record_dir=f'../nfs_folder/evaluation/step_by_step/chain_based/{args.GPU}/error_record_{model_size}2.json'
    ...
    for idx in tqdm.tqdm(range(args.start_data_idx, args.end_data_idx)):
        ...
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based/{args.GPU}/result/result_{idx}.json', 'w') as f:
            json.dump({'optimized_IR':optimized_IR, 'speedup':speedup}, f)
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based/{args.GPU}/database/database_{idx}.json', 'w') as f:
            json.dump(database, f)
        with open(f'../nfs_folder/evaluation/step_by_step/chain_based/{args.GPU}/node_history/history_{idx}.json', 'w') as f:
