...
def construct_prompt(node, step_by_step_prompt, start_description, basic_description, requirement, strategy_prompt, hardware_info):
    current_IR=node.state
    if node.different_original:
        current_applied_strategy=node.state_dict['applied_strategy']
        current_info="The speedup value of the current IR: "+str(node.speedup)+", and the current IR is obtained from the parent IR using the strategy \'"+current_applied_strategy+"\'"
    else:
        current_info="The current IR is the same as the root IR"
    if node.trials>0:
        ...
        if not node.different_original:
            ...
            if node.failed_children[-1][1]['different_original']==0:
                current_info+=f"Last time, the transformed IR, \'{node.failed_children[-1][0]}\', was same as the root IR. You MUST compare with this failed transformed IR when generating new IR. You are FORBIDDEN to generate this failed transformed IR again. \n"
            elif node.failed_children[-1][1]['different_original']==1:
                current_info+=f"Last time, the transformed IR, \'{node.failed_children[-1][0]}\', was same as the current IR. You MUST compare with this failed transformed IR when generating new IR. You are FORBIDDEN to generate this failed transformed IR again. \n"
            else:
                current_info+=f"Last time, the transformed IR, \'{node.failed_children[-1][0]}\', was not numerically equivalent to the original IR. You MUST compare with this failed transformed IR when generating new IR. You are FORBIDDEN to generate this failed transformed IR again. \n"
    else:
        current_info+='.\n'
    parent_info=""
    if node.parent:
        if node.different:
            if not node.parent.equal:
                parent_info="History:\nThe parent IR: \'"+str(node.parent.state_dict)+"\', non-equal IR.\n"
            elif 'original_IR' in str(node.parent.state_dict):
                parent_info="History:\nThe parent IR: same as the root IR, speedup value: 1. \n"
            else:
                parent_info="History:\nThe parent IR: \'"+str(node.parent.state_dict)+"\', speedup value: "+str(node.parent.speedup)+'\n'
        else:
...
class MCTSNode:
    def __init__(self, state, state_dict={}, parent=None):
        self.state = state  # string
        self.state_dict=state_dict
        self.parent = parent
        self.children = []  # store MCTSNode objects
    ...
    def uct_value(self, c_param=1.414):
        ...
        if self.visits == 0:
            return float('inf')  # explore untried nodes first
        # UCB1
        return (self.value / self.visits) + c_param * math.sqrt(2 * math.log(self.parent.visits) / self.visits)
...
class MCTS:
    def __init__(self, info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, veri_cuda, eval_idx, database, error_record_dir):
        self.info = info
    ...
    def search(self, original_IR, iterations):
        self.original_IR=original_IR
        root = MCTSNode(state=original_IR, state_dict={"original_IR": original_IR})

        for _ in range(iterations):
            #--------------selection---------------------
            leaf = self._select(root)
            # print(f'leaf.state:{leaf.state}')
            #--------------expansion---------------------
            if not self._is_terminal(leaf.state):
                target_node, IR_in_prompt = self._expand(leaf)
            else:
                target_node, IR_in_prompt = leaf, leaf.state
            
            #-------------simulation---------------------
            verification_result = self._simulate(target_node, IR_in_prompt)
            
            #-------------backpropagate------------------
            self._backpropagate(target_node, leaf, verification_result)
        try:
    ...
    def _expand(self, node):
        prompt=construct_prompt(node, self.step_by_step_prompt, self.start_description, self.basic_description, self.requirement, self.strategy_prompt, self.hardware_info)
        IR_in_prompt=node.state
        print(f'prompt:{prompt}')
        answer = infer(self.engine, InferRequest(messages=[{'role': 'user', 'content': prompt}]), self.temperature)
        print(f'answer:{answer}')
        answer_dict=obtain_answer_dict(answer)
        try:
            transformed_IR=answer_dict['transformed_IR']
        except:
            transformed_IR=answer
        # print(f'transformed_IR:{transformed_IR}\n IR_in_prompt:{IR_in_prompt}')
        target_node=MCTSNode(state=transformed_IR, state_dict=answer_dict)
        return target_node, IR_in_prompt
        
    def _simulate(self, node, IR_in_prompt):
    ...
    def _backpropagate(self, node, leaf, verification_result):
        #-----------------equal part-------------------
        if verification_result['equal_pass']>0:
            node.equal=True
        else:
            node.equal=False
        #-----------------different part-------------------
        if verification_result['different_pass']>0:
            node.different=True
        else:
            node.different=False
        if verification_result['different_original']>0:
            node.different_original=True
        else:
            node.different_original=False
        #----------------reward part------------------------
        if len(verification_result['tvm_time'])>0:
            avg_tvm=sum(verification_result['tvm_time'])/len(verification_result['tvm_time'])
            reward=self.original_tvm_time/avg_tvm
        else:
            reward=0
        #----------------expand node------------------------
        if node.different and node.different_original and reward>0:
            leaf.is_expanded=True
            node.parent=leaf
            leaf.children.append(node)
            node.speedup=reward
            self.speedup_dict[node.state]=[node.speedup, node]
            # print(f'reward:{reward}')
            value=(math.exp(reward-1)-1)*10
            while node:
                node.visits += 1
                node.value += value
                node = node.parent
            ...
            leaf.trials+=1
            leaf.failed_children.append([node.state_dict, verification_result])
    
    def _is_terminal(self, state):
        """
        whether this state is a terminal state
        """
        if self.repeat_num<=5:
            return False
        else:
            return True
...
if __name__ == '__main__':
    ...
    #--------------pre-settings----------------------------
    step_by_step_prompt="Monte-Carlo-Tree-Search-based optimization is used on a given IR to improve performance. Each IR is a state, and has a parent transformation and speedup performance."
    ...
    error_record_dir=f'../nfs_folder/evaluation/step_by_step/MCTS_based/{args.GPU}/error_record_{model_size}.json'
    ...
    for idx in tqdm.tqdm(range(args.start_data_idx, args.end_data_idx)):
        ...
        requirement=prompt[prompt.index('Please give me a **numerically equivalent transformed** IR'):].replace("numerically equivalent transformed", "numerically equivalent, runtime-performance-optimized").replace("(bitwise identical),", "(bitwise identical) to achieve higher speedup value (should be more than 1),")
        requirement="**Task**:\n"+requirement
        requirement+="\nCRITICAL:\n1. Before you suggest a new transformation, you MUST identify what has been changed in the current IR compared to the root IR. You MUST build your new optimization ON TOP OF these existing changes, namely ON TOP OF the current IR. The strategies MUST be used on the current IR! You MUST compare your modified parts in the transformed IR with the current IR. If they are identical strings, your answer is WRONG. If the unmodified part in transformed IR is different from the current IR, your answer is WRONG.\n2.Don't repeat the current or parent IRs! You MUST NOT revert to the parent IR: In particular, you are NOT allowed to apply any reverse or undo operation that reconstructs the current IR from its parent IR, including inverse transformations such as operator fusion <-> operator fission, loop tiling <-> loop fusion, loop split <-> loop fusion, apart <-> together, collect <-> expand collect, or similar reversals."
        num_IRs=1
        # print(f'start_description:{start_description}\n basic_description:{basic_description}\n requirement:{requirement}')
        msts=MCTS(info, step_by_step_prompt, hardware_info, strategy_prompt, num_IRs, start_description, basic_description, requirement, original_tvm_time, temperature, engine, 0, idx, database, error_record_dir)
        print("MCTS created")
        optimized_IR, speedup, final_node, database=msts.search(original_IR, max_iterations)
        ...
        with open(f'../nfs_folder/evaluation/step_by_step/MCTS_based/{args.GPU}/result/result_{idx}.json', 'w') as f:
            json.dump({'optimized_IR':optimized_IR, 'speedup':speedup}, f)
        with open(f'../nfs_folder/evaluation/step_by_step/MCTS_based/{args.GPU}/database/database_{idx}.json', 'w') as f:
            json.dump(database, f)
        with open(f'../nfs_folder/evaluation/step_by_step/MCTS_based/{args.GPU}/node_history/history_{idx}.json', 'w') as f:
